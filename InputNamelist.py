...
if "laser" in selected_case:
	...
	LaserEnvelopeGaussianAM(
        ...
        focus           = [x_focus_laser],                                          # laser focus, [x,r] position
...
if "wakefield" in selected_case:
	...
	def plasma_density(x,r):
		profile_r = np.ones_like(r)
		np.where(r>Radius_plasma,0,profile_r) 
...
if ("acceleration" in selected_case):
	...
	##### Diagnostic to track the electron bunch macro-particles
	DiagTrackParticles(
...
if (diags_for_3D_export==True):

	DiagFields( every = 100, fields = ["Env_E_abs"])
	DiagFields( every = 100, fields = ["Rho","Rho_plasmaelectrons"])
