...
\textbf{ - Exercise 2:} What are the precise longitudinal size \commandline{Lx} and radial size \commandline{Lr} of the simulation window (in code units and physical units)? How many mesh points are used? See Figures \ref{reference_axes}, \ref{Schema0} for reference, and find these lengths from the namelist.
...
\textbf{ - Exercise 3:} What are the waist size, FWHM duration in field of the laser (code and physical units) specified in the namelist? Where is the laser pulse center placed in the simulation window at the start of the simulation? Where is the focal plane of the laser placed?\\
...
What is the theoretical Rayleigh length $x_R$  (in physical and normalized units)?\\

\textbf{ - Exercise  6:} Use the script \commandline{laser\_waist\_theory\_vs\_Smilei.py} to compare the analytical Rayleigh law and the code results. Copy the script in the simulation folder. The script loads the results, then loops over the iterations available in output and computes the laser pulse waist $w(x)$ as 
\begin{equation}
...
\textbf{ - Exercise 7:} As you can see, the plasma density has a value $n_0=0.0008$ (in normalized units) in the uniform region.  What is its value in physical units (electrons/cm$^{-3}$)? \textit{Hint:} see Table \ref{tab_normalisations}. If you wanted to specify $n_0$ through a conversion from SI units to code units using the conversion with \commandline{ncrit} (like for example when defining the variable \commandline{laser\_waist}), what command should you use? \\
...
\textbf{ - Exercise 11:} Reading the namelist, provide a description of the electron bunch in SI units: total charge, energy, rms sizes along $x$, $y$, $z$, rms energy spread, emittance  along the transverse planes. Where is the electron bunch placed in the window initially (code and physical units)? \\
